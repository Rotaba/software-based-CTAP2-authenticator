...
def uhid_generate_input2_event(input_report_data):
    # print("uhid_generate_input2_event: ", input_report_data)
    ev_type = 12
...
def _process_request_initialization_packet(octets):
    global X
    # print('_process_request_initialization_packet')
    initpkt = InitPacket(*struct.unpack(INIT_PACKET_FMT, octets))
    # print("*** initpkt.cmd ***" + str(hex(initpkt.cmd)) +  " with data " + str(b2a_hex(initpkt.data)))
    if initpkt.cmd not in (U2FHID_INIT, U2FHID_PING, U2FHID_MSG, U2FHID_INFO):
    ...
    assert (( initpkt.cmd == U2FHID_INIT and initpkt.cid == 0xffffffff and
              initpkt.bcnt == 8 ) or
            ( initpkt.cmd in (U2FHID_PING, U2FHID_MSG, U2FHID_INFO) and
    ...
    if current_time < X.deadline:
        ...
        # print("golobal buffer X is: " + str(X))
    ...
    elif initpkt.cmd == U2FHID_INFO:
        print("U2FHID_INFO")
        _process_request_message_INFO(initpkt.cid, initpkt.data[:initpkt.bcnt])


def _process_request_continuation_packet(octets):
    global X
    # print('_process_request_continuation_packet with CONT_PACKET_FMT')
    ...
    # print("golobal buffer X is: " + str(X))
    # print("X.cmd" + str(X.cmd) + " and X.bcnt " + str(X.bcnt) + " and len(X.data) " + str(len(X.data)))
    if len(X.data) >= X.bcnt:
        if X.cmd == U2FHID_MSG:
            _process_request_message_MSG(X.cid, X.data[:X.bcnt])
        if X.cmd == U2FHID_INFO:
            _process_request_message_MSG(X.cid, X.data[:X.bcnt])
...
def _process_request_message_INIT(nonce):
    ...
    capabilities = 4 #b'\x04'
    # ~ payload = struct.pack('> 8s L B B B B B', nonce, new_cid, 2, 0, 0, 0, 0)
    payload = struct.pack('> 8s L B B B B B', nonce, new_cid, 2, 0, 0, 0, capabilities)
...
def _process_request_message_MSG(cid, data):
    ...
    resp = u2fraw.process_u2fraw_request(data) #create resp to request
    # _send_response_message(cid, U2FHID_MSG, resp)
    _send_response_message(cid, U2FHID_INFO, resp)

def _process_request_message_INFO(cid, data):
    print('U2FHID> got INFO request message cid=0x%08x data=[%s]' % (cid, data.hex()))
    resp = u2fraw.process_u2fraw_request(data) #create resp to request
    _send_response_message(cid, U2FHID_INFO, resp)

def _send_response_message(cid, cmd, payload):
    assert cmd in (U2FHID_INIT, U2FHID_PING, U2FHID_MSG, U2FHID_ERROR, U2FHID_INFO)
    assert len(payload) <= 7609
    #print('payload.hex(): ', payload.hex())
    print('U2FHID< send %s response message \ncid=0x%08x \ndata=[%s]' % (
        {
            U2FHID_INIT  : 'INIT',
            U2FHID_PING  : 'PING',
            U2FHID_MSG   : 'MSG',
    ...
    print("-----------------------done printing octets, response DONE-----------------------------------")
